...
class Vault {
    ...
    doFetch (method, path) {
        ...
        return fetch(url, this.fetchOptions(method)).then(v => v.json()).catch(console.error);
    }
    ...
    async fetchAllFrom (path = '') {
        if(matchArray(this.exclude, path) || this.tooDeep(path)) {
            return null;
        }
        ...
        debug('vault-fetch')(`[${this.responses} / ${this.requests} complete] Processing ${path}`);
